...
| 住宿三态与旅行骨架 | `trip_skeleton.py`（B3，transient 领域模型）：`AccommodationState` / `AreaEstimateSource` / `ConfirmedAccommodation` / `AreaEstimatedAccommodation` / `UnresolvedAccommodation` / `OvernightBoundary` / `TripSkeleton`；尚未进入 planning worker，也未进入消息契约、DB 或 API，投影与跨日规则接入属于 B4 |
