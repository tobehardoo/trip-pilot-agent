...
_ISO_DATE = re.compile(r"(?P<year>\d{4})[-/.年](?P<month>\d{1,2})[-/.月](?P<day>\d{1,2})日?")
_CHINESE_DATE = re.compile(r"(?P<month>\d{1,2})月(?P<day>\d{1,2})日(?:前后)?")


def normalize_trip_date(value: Any, *, today: date | None = None) -> date | None:
    """Normalize a confirmed date value into a real date, or None.

    Accepts ``datetime``/``date``, ISO dates (``2026-10-01`` / ``2026/10/01`` /
    ``2026年10月1日``) and the common Chinese ``M月D日`` form.  A bare
    month-day is read as this year — or next year once that day has passed —
    so a conversation in December about ``1月1日`` resolves forward.  The
    Chinese form is extracted from surrounding free text (a spoken scan
    passes fragments like "想去成都玩…10月1日"), while ISO dates must match
    the whole value.

    Single source for the agent itinerary builder and the dialog extractor;
    both previously carried their own copy of this logic.
    """
    if value is None:
        return None
    ...
    if iso:
        try:
            ...
            return None
    chinese = _CHINESE_DATE.search(text)
    if chinese:
        try:
            parsed = date(
                reference.year, int(chinese.group("month")), int(chinese.group("day"))
            )
        except ValueError:
            return None
        if parsed < reference:
