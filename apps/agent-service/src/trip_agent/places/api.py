...
def _resolved_provider_mode() -> ProviderExecutionMode:
    raw = os.getenv("PROVIDER_MODE", "").strip().upper()
    if raw:
        return ProviderExecutionMode(raw)
    demo = os.getenv("DEMO_MODE", "").strip().lower()
...
def create_place_search_runtime() -> PlaceSearchRuntime:
    ...
    if _resolved_provider_mode() == ProviderExecutionMode.DEMO_ONLY:
