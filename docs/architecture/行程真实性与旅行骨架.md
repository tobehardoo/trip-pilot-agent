...
- `TripSkeleton` 目前**仅存在于 `PlanningResult` 内存对象**；worker、消息契约（v8 completion）、DB 与 API 均不消费该字段。
- `ROUTE_ENDPOINT_CONTINUITY` / `CROSS_DAY_CONTINUITY` 仍未实现，属于 B4B。
- 因此**不能**声称 Hard Validation 或住宿闭环已完成；continuity 规则对 `AREA_ESTIMATED` / `UNRESOLVED` 只能产生 UNKNOWN → UNVERIFIED 的语义仍然有效。
...
只有 `CONFIRMED` 提供精确住宿锚点；`AREA_ESTIMATED` 与 `UNRESOLVED` 对跨日连续性规则只能形成 UNKNOWN，报告为 UNVERIFIED，不能持久化为 VERIFIED 正式版本。
