...
def test_local_replanning_only_rebuilds_impacted_transit() -> None:
    ...
    replan_service = import_module("trip_agent.application.replan_service")
    command = contracts.PlanningReplanCommand.model_validate(REPLAN_COMMAND)
    ...
    completed = asyncio.run(
        processor.process_planning_replan(
            command,
            replan_service.LocalReplanningProvider(route_provider),
...
def test_local_replanning_matches_reordered_transit_legs_by_endpoints() -> None:
    ...
    completed = asyncio.run(
        processor.process_planning_replan(
            command,
            replan_service.LocalReplanningProvider(RouteProvider()),
            occurred_at=datetime(2026, 7, 24, 4, 2, tzinfo=UTC),
        )
    )
...
def test_local_replanning_provider_accepts_an_optional_transit_route() -> None:
    ...
    provider = replan_service.LocalReplanningProvider(
...
def test_replan_routes_a_transit_leg_through_the_transit_provider() -> None:
    ...
    completed = asyncio.run(
        processor.process_planning_replan(
            command,
            replan_service.LocalReplanningProvider(
                route_provider,
