...
def test_implemented_rule_ids_equal_the_full_required_set() -> None:
    assert IMPLEMENTED_RULE_IDS == REQUIRED_RULE_IDS
    assert set(IMPLEMENTED_RULE_IDS).issubset(set(REQUIRED_RULE_IDS))
    assert len(set(IMPLEMENTED_RULE_IDS)) == len(IMPLEMENTED_RULE_IDS) == 11


def test_missing_rule_ids_is_empty() -> None:
