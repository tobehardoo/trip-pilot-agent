...
> 本文是 2026-07-19 至 2026-07-23 的历史架构审计记录。部分当时的 P0 缺口已经完成。
> 当前评分和后续需求以
> [产品完整度与需求基线](27-product-completeness-and-requirements-baseline.md) 为准。
...
3. 官方采集闭环已完成；下一阻塞点是把已发布知识、引用与 freshness 真正接入规划结果。
4. 动态事实（营业时间、票价、预约、天气、交通）还没有统一的“必须实时核验”执行策略。

### P1：生产可靠性和安全不足

1. Refresh Token 已迁移到 `HttpOnly + Secure + SameSite=Strict` Cookie；浏览器 JavaScript 和 JSON 响应不再接触刷新令牌。
...
- `knowledge_resource`、`knowledge_snapshot`、`knowledge_fetch_run` 已于 2026-07-22 完成，`AcquisitionWorkflow` 通过校验器及其对应内容哈希强制组合条件调度与记录，具备单事务写入、304/失败隔离、内容与解析器版本幂等、内容回退识别和实际并发/乱序完成保护。
- 官方正文抽取、第一版内容质量规则与解析结果版本化持久化已于 2026-07-22 完成；人工审核队列、追加式审计、审核状态机、唯一幂等发布适配器和按来源 freshness report 已于 2026-07-23 完成，P0-1 关闭。

### P0-2：把 RAG 接入规划结果

- 固定广州评测集、文档级 Recall@K/MRR、日期/语料版本固定和 `DEMO_ONLY` 防误判已于 2026-07-23 完成；取得供应商凭据后用同一门禁比较真实 Embedding/Rerank，不再把 `demo-hash-v1` 用于质量判断。
- Python Worker 返回 `KnowledgeCitation` 和 `freshness`，Java 持久化行程使用的文档/片段版本。
- Vue 展示推荐理由来源、来源时间和 Demo/真实状态。

### P0-3：补齐真实规划能力

- 确定性 V1 已完成候选 POI 去重、偏好评分、可解释淘汰、时间/预算/固定安排/交通硬约束，以及不可行冲突和最小放宽建议。
- 当前预算基于明确标记的活动费用估算；真实票价接入前不得把估算值宣传为供应商报价。
- 结构化模型输出和失败修复循环不属于确定性 V1 的运行依赖，留到真实模型评测门禁通过后实施。

### P1：生产安全与可运维

- Refresh Token Cookie 化、CSP、限流、任务配额和取消语义。
- Prometheus 基础设施/JVM/HTTP 抓取已接入；任务成功率、队列积压、Provider 延迟、模型成本、RAG 新鲜度仍是下一阶段业务指标门禁。
...
- Phase 12 条件 HTTP 获取已落地，具备 304、拒绝压缩编码的原始字节流上限、显式重定向复核和错误分类；DNS 全结果公网单播校验、单次抓取 IP 固定、环境代理隔离、每来源限速与有上限重试已完成。资源、候选快照、采集运行和版本化解析结果持久化已通过真实 PostgreSQL 验收，广州官方 HTML 正文/发布时间抽取与第一版质量规则已通过 3 个真实页面验收；人工审核队列、批准/拒绝/发布前撤回审计、文档版本分配、唯一 `KnowledgeImporter` 发布适配器和只读 freshness report 已完成，P0-1 已关闭。
