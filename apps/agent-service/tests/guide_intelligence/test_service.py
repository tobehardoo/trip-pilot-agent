...
from trip_agent.guide_intelligence.ocr import ImagePayload
...
def _image_payload() -> ImagePayload:
    ...
    buffer = io.BytesIO()
    Image.new("RGB", (96, 96), color=(10, 99, 180)).save(buffer, format="PNG")
    return ImagePayload(dataBase64=base64.b64encode(buffer.getvalue()).decode("ascii"))


def _png_bytes_for_url() -> bytes:
    import io

    from PIL import Image
    buffer = io.BytesIO()
    Image.new("RGB", (480, 320), color=(30, 30, 30)).save(buffer, format="PNG")
    return buffer.getvalue()


def _fetched_image(image_bytes: bytes, content_type: str | None):
    return ResourceFetched(
        status="FETCHED",
        requested_url="https://img.example.com/guide-shot.png",
        final_url="https://img.example.com/guide-shot.png",
        fetched_at=datetime(2026, 8, 2, 8, 0, tzinfo=UTC),
        content=image_bytes,
...
def test_import_url_routes_image_payloads_through_the_ocr_pipeline() -> None:
    provider = StubOcrProvider(
        "陈家祠地址：广州市荔湾区中山七路恩龙里34号。\n"
        "开放时间：09:00-17:30，成人门票10元，需要提前预约。"
    )
    result = __import__("asyncio").run(
        _import_url_with_fetcher(
            _fetched_image(_png_bytes_for_url(), "image/png"),
            ocr_provider=provider,
        )
    )
    assert result.source_type == "IMAGE_OCR"
    assert result.source_url == "https://img.example.com/guide-shot.png"
    assert result.source_host == "img.example.com"
    document = result.normalized_document
    assert document is not None
    assert document.metadata["ocr"]["imageUrl"] == "https://img.example.com/guide-shot.png"
    assert {fact.category for fact in result.trusted_facts} >= {
        "ADDRESS",
        "OPENING_HOURS",
        "TICKET_PRICE",
...
def test_import_url_sniffs_image_magic_even_when_content_type_lies() -> None:
    provider = StubOcrProvider("广州塔地址是阅江西路222号，门票约150元，建议提前购票。")
    result = __import__("asyncio").run(
        _import_url_with_fetcher(
            _fetched_image(_png_bytes_for_url(), "application/octet-stream"),
            ocr_provider=provider,
        )
    )
    assert result.source_type == "IMAGE_OCR"
...
async def _import_url_with_fetcher(fetched, *, ocr_provider) -> object:
    service = GuideImportService(
        fetcher=StubFetcher(fetched),
        ocr_provider=ocr_provider,
    )
    return await service.import_url(fetched.requested_url)


def test_import_url_image_still_requires_configured_ocr_provider(monkeypatch) -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    ...
    with __import__("pytest").raises(OcrError) as excinfo:
    ...
    assert excinfo.value.code == "OCR_NOT_CONFIGURED"


class StubScanExtractor:
    def __init__(self, text=None, error=None):
        self.text = text
        self.error = error
        self.images = []
...
class StubTextRefiner:
    ...
    async def refine(self, raw_text: str) -> str:
        ...
        return self.text or raw_text


def _import_images_with(*, ocr_provider=None, scan_extractor=None, text_refiner=None):
    return __import__("asyncio").run(
        GuideImportService(
            ocr_provider=ocr_provider,
            scan_extractor=scan_extractor,
            text_refiner=text_refiner,
        ).import_images(images=[_image_payload()])
    )


def test_vision_failure_falls_back_to_scan_then_llm_refinement() -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    provider = StubOcrProvider(error=OcrError("OCR_FAILED", "视觉模型不可用"))
    scan = StubScanExtractor(
        text="陈家祠 地址 中山七路恩龙里34号 09:00-17:30 门票10元 需要提前预约"
    )
    refiner = StubTextRefiner(
        "陈家祠地址：中山七路恩龙里34号。\n开放时间：09:00-17:30，成人门票10元，需要提前预约。"
    )
    result = _import_images_with(
        ocr_provider=provider,
        scan_extractor=scan,
        text_refiner=refiner,
    )
    assert result.source_type == "IMAGE_OCR"
    assert result.normalized_document.metadata["ocr"]["provider"] == "scan_fallback"
    assert result.normalized_document.metadata["ocr"]["fallbackReason"] == "OCR_FAILED"
    assert refiner.inputs and "中山七路" in refiner.inputs[0]
    assert {fact.category for fact in result.trusted_facts} >= {
        "ADDRESS",
        "OPENING_HOURS",
        "TICKET_PRICE",
...
def test_scan_fallback_uses_raw_text_when_no_refiner_configured(monkeypatch) -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    ...
    provider = StubOcrProvider(error=OcrError("OCR_TIMEOUT", "超时"))
    scan = StubScanExtractor(
        text="广州塔地址是阅江西路222号，门票约150元，建议提前线上购票。"
    )
    result = _import_images_with(
        ocr_provider=provider,
        scan_extractor=scan,
    ...
    assert result.normalized_document.metadata["ocr"]["provider"] == "scan_fallback"
    assert result.normalized_document.metadata["ocr"]["refinedByLLM"] is False
    assert "阅江西路222号" in result.normalized_document.content


def test_scan_fallback_disabled_keeps_original_vision_error(monkeypatch) -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    monkeypatch.setenv("OCR_FALLBACK_SCAN_ENABLED", "false")
    provider = StubOcrProvider(error=OcrError("OCR_TIMEOUT", "超时"))
    scan = StubScanExtractor(text="扫描文字")
    with __import__("pytest").raises(OcrError) as excinfo:
        _import_images_with(
            ocr_provider=provider,
            scan_extractor=scan,
...
def test_unavailable_scan_engine_surfaces_clear_error() -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    provider = StubOcrProvider(error=OcrError("OCR_FAILED", "视觉失败"))
    scan = StubScanExtractor(error=OcrError("OCR_SCAN_UNAVAILABLE", "扫描组件未安装"))
    with __import__("pytest").raises(OcrError) as excinfo:
        _import_images_with(
            ocr_provider=provider,
            scan_extractor=scan,
        )
    assert excinfo.value.code == "OCR_SCAN_UNAVAILABLE"

def test_import_images_runs_ocr_text_through_trusted_fact_pipeline(monkeypatch) -> None:
    monkeypatch.delenv("STRUCTURED_MODEL_ENDPOINT", raising=False)
    monkeypatch.delenv("STRUCTURED_MODEL_API_KEY", raising=False)
    monkeypatch.delenv("STRUCTURED_MODEL_NAME", raising=False)
    provider = StubOcrProvider(
    ...
    result = _import_images_with(ocr_provider=provider)
    assert result.source_type == "IMAGE_OCR"
    assert result.source_host == "用户图片截图"
    assert len(provider.images) == 1
    document = result.normalized_document
    assert document is not None
    assert document.metadata["ocr"]["imageCount"] == 1
    assert len(document.metadata["ocr"]["images"][0]["sha256"]) == 64
    assert {fact.category for fact in result.trusted_facts} >= {
        "ADDRESS",
        "OPENING_HOURS",
        "TICKET_PRICE",
    ...
    assert all(
        fact.evidence == document.content[fact.evidence_start : fact.evidence_end]
        for fact in result.trusted_facts
    )
    assert result.model_extraction.status == "SKIPPED"
    assert result.model_extraction.failure_code == "MODEL_NOT_CONFIGURED"


def test_import_images_requires_configured_ocr_provider(monkeypatch) -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    monkeypatch.delenv("OCR_MODEL_ENDPOINT", raising=False)
    monkeypatch.delenv("OCR_MODEL_API_KEY", raising=False)
    monkeypatch.delenv("OCR_MODEL_NAME", raising=False)
    with __import__("pytest").raises(OcrError) as excinfo:
        __import__("asyncio").run(GuideImportService().import_images(images=[_image_payload()]))
    assert excinfo.value.code == "OCR_NOT_CONFIGURED"
    assert "未配置" in excinfo.value.message


def test_import_images_reports_when_no_text_is_recognized() -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    provider = StubOcrProvider(text="")
    with __import__("pytest").raises(OcrError) as excinfo:
        _import_images_with(ocr_provider=provider)
    assert excinfo.value.code == "OCR_NO_TEXT"


def test_import_images_rejects_too_short_ocr_text() -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    provider = StubOcrProvider(text="你好")
    with __import__("pytest").raises(OcrError) as excinfo:
        _import_images_with(ocr_provider=provider)
    assert excinfo.value.code == "OCR_TEXT_TOO_SHORT"


def test_import_images_propagates_provider_errors() -> None:
    from trip_agent.guide_intelligence.ocr import OcrError
    provider = StubOcrProvider(error=OcrError("OCR_TIMEOUT", "超时"))
    with __import__("pytest").raises(OcrError) as excinfo:
        _import_images_with(ocr_provider=provider)
