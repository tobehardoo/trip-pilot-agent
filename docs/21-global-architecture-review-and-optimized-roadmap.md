...
3. 官方采集已完成受控 HTTP、快照、正文抽取、人工审核与幂等发布，尚缺按来源暴露过期原因的 freshness report。
...
- 官方正文抽取、第一版内容质量规则与解析结果版本化持久化已于 2026-07-22 完成；人工审核队列、追加式审计、审核状态机和唯一幂等发布适配器已于 2026-07-23 完成，过期报告仍待完成。
...
- Phase 12 条件 HTTP 获取已落地，具备 304、拒绝压缩编码的原始字节流上限、显式重定向复核和错误分类；DNS 全结果公网单播校验、单次抓取 IP 固定、环境代理隔离、每来源限速与有上限重试已完成。资源、候选快照、采集运行和版本化解析结果持久化已通过真实 PostgreSQL 验收，广州官方 HTML 正文/发布时间抽取与第一版质量规则已通过 3 个真实页面验收；人工审核队列、批准/拒绝/发布前撤回审计、文档版本分配和唯一 `KnowledgeImporter` 发布适配器已完成，P0-1 只剩 freshness report。
