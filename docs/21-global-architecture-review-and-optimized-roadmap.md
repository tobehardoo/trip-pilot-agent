...
- Phase 12 条件 HTTP 获取已落地，具备 304、拒绝压缩编码的原始字节流上限、显式重定向复核和错误分类；DNS 全结果公网单播校验、单次抓取 IP 固定、环境代理隔离、每来源限速与有上限重试已完成。资源、候选快照和采集运行持久化已通过真实 PostgreSQL 验收，正文抽取、质量检查与审核发布仍未完成。
