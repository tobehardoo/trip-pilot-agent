...
public class PlanningCompletedEventParser {
    ...
    public PlanningCompletedEvent parse(byte[] body) {
        try {
            ...
            throw new EventContractException("Invalid PLANNING_COMPLETED event", exception);
        }
    }
    ...
    private void validateJsonTypes(JsonNode event) {
        ...
        validateProviderProvenanceTypes(payload);
        validateEvaluationTypes(payload);
        validateFeasibilityReportTypes(payload);
        for (JsonNode day : days) {
            ...
            for (JsonNode activity : activities) {
                ...
                if (activity.has("activityId") && !activity.path("activityId").isNull()
                    ...
                    throw invalid("activityId is only supported as a UUID string in schema v9/v10/v11");
                }
            }
    ...
    private void validateProviderProvenanceTypes(JsonNode payload) {
    ...
    private void validateTransitLegTypes(JsonNode day, int schemaVersion) {
        ...
        for (JsonNode leg : transitLegs) {
            ...
            if (leg.has("transitId") && !leg.path("transitId").isNull()
                ...
                throw invalid("transitId is only supported as a UUID string in schema v9/v10/v11");
            }
            ...
            if (leg.has("estimatedCost") && !leg.path("estimatedCost").isNull()
                    && !ItineraryContractValidator.isPersistableMoney(leg.path("estimatedCost").decimalValue())) {
    ...
    private void validate(PlanningCompletedEvent event) {
        ...
        if (event.payload() == null || event.payload().itinerary() == null
                || !ItineraryContractValidator.supportedProvider(event.payload().provider())) {
            throw invalid("supported payload is required");
        }
        PlanningCompletedEvent.Itinerary itinerary = event.payload().itinerary();
        if (!ItineraryContractValidator.validText(itinerary.title(), 200)) {
        ...
        if (!ItineraryContractValidator.isPersistableMoney(itinerary.estimatedTotalCost())) {
        ...
        validateKnowledge(event.payload().knowledge());
        validateFactImpacts(event.payload().factImpacts());
        validateProviderProvenance(event);
        validateEvaluation(event);
        validateFeasibilityReport(event);
    }

    private void validateFeasibilityReportTypes(JsonNode payload) {
        JsonNode report = payload.path("feasibilityReport");
        if (!report.isObject()
    ...
    private void validateEvaluationTypes(JsonNode payload) {
    ...
    private void validateEvaluation(PlanningCompletedEvent event) {
        ...
        if (evaluation.schemaVersion() != 1 && evaluation.schemaVersion() != 2) {
        ...
        if (!ItineraryContractValidator.validText(evaluation.summary(), 1_000)
        ...
        if (evaluation.warnings() != null) {
            for (PlanningCompletedEvent.EvaluationWarning w : evaluation.warnings()) {
                if (w == null
                        ...
                        || !ItineraryContractValidator.validText(w.message(), 300)
        ...
        if (evaluation.decisions() != null) {
            ...
            for (PlanningCompletedEvent.DecisionExplanation d : evaluation.decisions()) {
                if (d == null
                        ...
                        || !ItineraryContractValidator.validText(d.summary(), 300)
                        ...
                        || d.reasons().stream().anyMatch(reason -> !ItineraryContractValidator.validText(reason, 300))
                        || d.evidence().stream().anyMatch(evidence -> evidence == null
                                || !ItineraryContractValidator.validText(evidence.key(), 60)
                                || !ItineraryContractValidator.validText(evidence.label(), 300)
                                || !ItineraryContractValidator.validText(evidence.value(), 200))) {
    ...
    private void validateProviderProvenance(PlanningCompletedEvent event) {
        ...
        if (provenance.requestedProviderMode() == null
    ...
    private boolean validCode(String value) {
        return ItineraryContractValidator.validText(value, 60) && value.matches("[A-Z0-9_]+");
    }

    private void validateFactImpacts(List<PlanningCompletedEvent.FactImpact> impacts) {
        if (impacts.size() > 500) {
            throw invalid("fact impacts exceed the supported limit");
        }
        for (PlanningCompletedEvent.FactImpact impact : impacts) {
            if (impact == null
                    || !ItineraryContractValidator.validText(impact.factId(), 80)
                    || !ItineraryContractValidator.validText(impact.category(), 60)
                    || !ItineraryContractValidator.validText(impact.effect(), 60)
                    || !ItineraryContractValidator.validText(impact.reason(), 300)
                    || !ItineraryContractValidator.validText(impact.sourceName(), 120)
                    || !ItineraryContractValidator.validText(impact.sourceType(), 60)
                    || impact.sourceUrl() != null
                        && !ItineraryContractValidator.validHttpUrl(impact.sourceUrl())
                    || !ItineraryContractValidator.validText(impact.reliabilityLevel(), 60)
                    || impact.checkedAt() == null
                    || !ItineraryContractValidator.validText(impact.evidence(), 2000)
                    || impact.targetPoiId() != null
                        && !ItineraryContractValidator.validText(impact.targetPoiId(), 100)
                    || impact.targetName() != null
                        && !ItineraryContractValidator.validText(impact.targetName(), 120)) {
    ...
    private void validateKnowledge(PlanningCompletedEvent.KnowledgeEvidence knowledge) {
        if (knowledge == null || !ItineraryContractValidator.validText(knowledge.query(), 200)
        ...
        if (real) {
            if (knowledge.citations().isEmpty() || knowledge.message() != null
                ...
                || !ItineraryContractValidator.validText(knowledge.message(), 300)
                || !"UNAVAILABLE".equals(knowledge.freshness().status())) {
    ...
    private void validateFreshness(PlanningCompletedEvent.KnowledgeFreshness freshness) {
        ...
        if (!("FRESH".equals(freshness.status()) || "STALE".equals(freshness.status()))
                ...
                || (freshness.staleReason() != null && !ItineraryContractValidator.validText(freshness.staleReason(), 60))) {
    ...
    private boolean validCitation(PlanningCompletedEvent.KnowledgeCitation citation) {
        return citation != null
                && ItineraryContractValidator.validText(citation.documentId(), 200)
                && citation.documentVersion() >= 1
                && ItineraryContractValidator.validText(citation.chunkId(), 200)
                && citation.chunkIndex() >= 0
                && ItineraryContractValidator.validText(citation.title(), 200)
                && ItineraryContractValidator.validHttpUrl(citation.sourceUrl())
                && ItineraryContractValidator.validText(citation.sourceName(), 120)
                && citation.collectedAt() != null
                && ItineraryContractValidator.validText(citation.reliabilityLevel(), 60)
    ...
    private void validateDay(PlanningCompletedEvent.Day day, int schemaVersion, String provider) {
        ...
        if (day.dayType() != null && !ItineraryContractValidator.DAY_TYPES.contains(day.dayType())) {
        ...
        for (PlanningCompletedEvent.Activity activity : day.activities()) {
            if (activity == null || !ItineraryContractValidator.validText(activity.title(), 200)
                    || activity.startTime() == null || activity.endTime() == null
                    || !ItineraryContractValidator.isPersistableMoney(activity.estimatedCost())
                    || !ItineraryContractValidator.supportedProvider(activity.source())) {
                throw invalid("activity fields are invalid");
            }
            if (activity.kind() != null && !ItineraryContractValidator.ACTIVITY_KINDS.contains(activity.kind())) {
    ...
    private void validateTransitLegs(PlanningCompletedEvent.Day day, int schemaVersion) {
        // Gaps between adjacent activities are allowed (unresolved structural
        // nodes such as a meal without a bound restaurant have no transit
        // leg); a day may only never carry more legs than adjacent pairs.
    ...
    private void validateV2ActivitySource(PlanningCompletedEvent.Activity activity) {
        ...
        if (!ItineraryContractValidator.validText(activity.providerPoiId(), 100)
                || !ItineraryContractValidator.validText(activity.address(), 300)
    ...
    private EventContractException invalid(String detail) {
        return new EventContractException("Invalid PLANNING_COMPLETED event: " + detail);
    }
}
