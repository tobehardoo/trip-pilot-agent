...
import type { EvaluationDimensions, EvaluationWarning } from '../src/lib/api'
import {
...
} from '../src/lib/plan-evaluation-presentation'
...
describe('统一词汇', () => {
  // 契约新增维度时这个 fixture 会先编译失败，避免面板静默少一栏。
  const contractDimensions: EvaluationDimensions = {
  ...
  it('covers every contract dimension exactly once', () => {
    expect(EVALUATION_DIMENSIONS.map((d) => d.key).sort()).toEqual(
      Object.keys(contractDimensions).sort(),
    )
  })

  it('renders no raw enum as a label', () => {
    for (const { key, label } of EVALUATION_DIMENSIONS) expect(label).not.toBe(key)
    for (const severity of ['INFO', 'WARNING', 'CRITICAL'] as const) {
      expect(warningSeverityLabel(severity)).not.toBe(severity)
    }
    for (const subject of ['PLAN', 'DAY', 'ACTIVITY', 'TRANSIT'] as const) {
      expect(decisionSubjectLabel(subject)).not.toBe(subject)
    }
  })
})
