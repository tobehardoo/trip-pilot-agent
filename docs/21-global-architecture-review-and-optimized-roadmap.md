...
3. 官方采集只有来源注册表和固定 URL 发现，没有 HTTP 条件请求、快照、正文抽取、审核发布和 freshness report。
...
- HTTPX 条件请求：ETag、Last-Modified、超时、限速、重试。
- DNS 解析固定、私网地址阻断、重定向重新校验。
...
本机验证补充：当前 Windows + BellSoft JDK 21 环境下，Surefire fork 启动 JaCoCo native agent 会在测试进程启动前失败；使用 `mvn --batch-mode -pl apps/travel-server clean verify "-Djacoco.skip=true"` 可真实执行 88 个 Java 测试，但不产生覆盖率。正式覆盖率仍以 GitHub Actions 的 Linux JaCoCo 门禁为准，这个本地差异需要在开发环境标准化时消除。
