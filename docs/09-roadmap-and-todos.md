...
同日完成 Phase 12 第七小步：新增 V2 `knowledge_extraction`、`SnapshotExtractionService` 与仓储读写端口，按快照和解析器版本不可变保存正文/正文哈希/元数据/质量警告或结构化拒绝，稳定结果指纹保证重复执行 `unchanged`、合法漂移结果拒绝覆盖；提取 ID、正文哈希和结果指纹均从载荷重算，发布时间必须含时区并统一为 UTC，`EXTRACTED` 禁止 ERROR、`REJECTED` 必须包含 ERROR，服务层与 JSONB 约束双重执行。待处理查询只选择仍为 `PENDING` 且缺少目标解析器版本的快照，解析器升级会重新入队。新增 10 项服务与真实 PostgreSQL 覆盖，采集相关 124 项、Python 全量 263 项通过，知识检索与采集覆盖率 92.81%。下一步建立人工审核状态和审核发布适配器。
