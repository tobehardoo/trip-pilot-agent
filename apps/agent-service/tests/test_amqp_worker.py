...
def test_infeasible_plan_publishes_an_actionable_failure_and_acks() -> None:
    ...
    class InfeasibleProvider:
        async def plan(self, command: object):
            del command
            raise protocols.PlanningInfeasibleError(
                (
...
def test_provider_failure_publishes_v2_and_does_not_requeue() -> None:
    ...
    class FailedProvider:
        async def plan(self, command: object):
            del command
            raise protocols.PlanningProviderError(
    ...
    def handle(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                ...
                # V2 semantics: meal resolution only binds dining-class POIs,
                # so the mock carries a real restaurant.  It sits far from the
                # attractions, so the meal leg requires an actual route call —
                # the condition the leg assertions below exercise.  (Pre-V2
                # the leaky resolver reused a scenic POI as the restaurant.)
                + [
                    {
                        "id": "poi-restaurant",
                        "name": "真实餐厅",
                        "location": "113.40,23.20",
                        "type": "餐饮服务",
                        "typecode": "050000",
                        "pname": "广东省",
                        "cityname": "广州市",
