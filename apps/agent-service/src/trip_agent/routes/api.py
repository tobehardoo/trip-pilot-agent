...
def create_route_runtime(settings: WorkerSettings | None = None) -> RouteApiRuntime:
    # The HTTP application has historically treated an explicit PROVIDER_MODE
    # as authoritative over the deprecated DEMO_MODE switch (the places API
    # follows the same rule).  Keep the worker's stricter conflict check for
    # worker startup, but do not let a stale legacy value prevent the API from
    # starting when its explicit mode is valid.
    explicit_mode = os.getenv("PROVIDER_MODE", "").strip()
    resolved = settings or (
        WorkerSettings(demo_mode=None) if explicit_mode else WorkerSettings()
    )
