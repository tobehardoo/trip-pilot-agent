...
class CandidateRanker:
    ...
    def rank(
        ...
        *,
        ...
        # P1-2: budget-aware ranking.  ``cost_hints`` carries the pre-resolved
        # (pure, no I/O) cost per candidate; ``budget_ceiling`` is the
        # per-activity affordability ceiling (None → no budget stated, or the
        # budget is not tight).
    ...
    def _score(
        ...
        # P1-2: a tight budget demotes candidates whose *known* price exceeds
        # the per-activity ceiling.  Only PROVIDER-priced candidates are
        # judged — a fallback estimate is not evidence that a place is
        # expensive, and an unknown cost must never be penalised.
        if (
            budget_ceiling is not None
            and cost_hint is not None
            and cost_hint.source == "PROVIDER"
            and cost_hint.amount > budget_ceiling
        ):
            score -= 30
            reasons.append("BUDGET_TIGHT_COST_PENALTY")
