...
# Categories whose recovery candidate is a bounded retry.  QUOTA_EXCEEDED is
# transient (a retry may recover) but real-to-demo fallback stays
# configuration-gated — see ProviderFallbackPolicy._explicitly_allowed,
# which derives from this set minus QUOTA_EXCEEDED.
...
class ProviderFallbackPolicy:
    ...
    _explicitly_allowed = TRANSIENT_CATEGORIES - {
        ProviderErrorCategory.QUOTA_EXCEEDED
    }
    _local = frozenset(
        {
