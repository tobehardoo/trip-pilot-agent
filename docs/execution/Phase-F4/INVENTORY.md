...
| D-9 | TripSkeleton docstring 过时 | planning/trip_skeleton.py:22-24 | 3.0 §2.1："has not entered worker runtime" 不准确（processor.py:16-17,151-173 已接线） | ✅ **已修正**（F-4.4）：更新为"transient aggregate, consumed by validation path (validation_projection.build_trip_skeleton in worker pipeline), never in message contracts/persistence/API" | 无需动作 |
| D-10 | DecisionTrace docstring 过时 | planning/decision_trace.py:7-9 | 3.0 §2.2："so-far-unused vocabulary" 不准确（planning_provider.py:423 已构建 traces） | ✅ **已修正**（F-4.4）：更新为 reason-code vocabulary 已由 providers 的 reason_codes（PROVIDER_CONSTRAINT/BUDGET_CONSTRAINT/INTEREST_MATCH/MUST_VISIT/PACE_POLICY）接入真实决策 | 无需动作 |
