...
def configured_structured_extractor(
    ...
    endpoint = os.getenv("STRUCTURED_MODEL_ENDPOINT", "").strip()
    api_key = os.getenv("STRUCTURED_MODEL_API_KEY", "").strip()
    model = os.getenv("STRUCTURED_MODEL_NAME", "").strip()
    ...
    return StructuredModelFactExtractor(
        ...
        timeout_seconds=float(os.getenv("STRUCTURED_MODEL_TIMEOUT_SECONDS", "8")),
        max_retries=int(os.getenv("STRUCTURED_MODEL_MAX_RETRIES", "1")),
        max_input_characters=int(
            os.getenv("STRUCTURED_MODEL_MAX_INPUT_CHARACTERS", "30000")
        ),
    )
