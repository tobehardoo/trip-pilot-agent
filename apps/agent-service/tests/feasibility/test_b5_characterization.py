...
from trip_agent.planning.poi_quality import DurationProfile
...
def test_validator_version_is_v3() -> None:
    assert VALIDATOR_VERSION == "hard-validator-v3"


def test_duration_profile_has_eight_fields() -> None:
    fields = tuple(DurationProfile.__dataclass_fields__)
    assert fields == (
        "min_minutes",
        "recommended_minutes",
        "max_minutes",
