...
- 官方正文抽取、第一版内容质量规则与解析结果版本化持久化已于 2026-07-22 完成；人工审核、审核发布和过期报告仍待完成。
...
- Phase 12 条件 HTTP 获取已落地，具备 304、拒绝压缩编码的原始字节流上限、显式重定向复核和错误分类；DNS 全结果公网单播校验、单次抓取 IP 固定、环境代理隔离、每来源限速与有上限重试已完成。资源、候选快照、采集运行和版本化解析结果持久化已通过真实 PostgreSQL 验收，广州官方 HTML 正文/发布时间抽取与第一版质量规则已通过 3 个真实页面验收；人工审核与审核发布仍未完成。
