...
import { aggregateFactImpacts, factCategoryLabel, summarizeFactStatus } from '../src/lib/fact-status-presentation'
...
describe('summarizeFactStatus', () => {
  ...
  it('unknown categories still surface as待确认 with an action', () => {
    ...
    expect(summary.issues[0].message).toBe('1 项其他行程信息待确认')
    expect(summary.issues[0].message).not.toContain('POI_DETAILS')
...
describe('factCategoryLabel', () => {
  it('maps every known category to user wording regardless of spelling', () => {
    expect(factCategoryLabel('OPENING_HOURS')).toBe('营业时间')
    expect(factCategoryLabel('opening-hours')).toBe('营业时间')
    expect(factCategoryLabel('WEATHER')).toBe('天气')
    expect(factCategoryLabel('transit')).toBe('路线')
  })

  it('falls back to generic wording instead of leaking the raw enum', () => {
    expect(factCategoryLabel('POI_DETAILS')).toBe('其他行程信息')
    expect(factCategoryLabel('')).toBe('其他行程信息')
  })
})
