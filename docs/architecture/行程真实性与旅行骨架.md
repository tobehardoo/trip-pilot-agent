...
- `TripSkeleton` 仍为 transient：**仅存在于 `PlanningResult` 内存对象**；worker 运行时、消息契约（v8 completion）、DB 与 API 均不消费；B4B 未接 worker、消息、持久化或正式版本门禁。
- remaining rules = `MUST_VISIT_COVERAGE` / `OPENING_HOURS` / `VISIT_DURATION` / `MEAL_WINDOW`（implemented=7 / required=11）。
- 因此**不能**声称 Hard Validation 已完成；continuity 规则对 `AREA_ESTIMATED` / `UNRESOLVED` 只能产生 UNKNOWN → UNVERIFIED 的语义仍然有效。
