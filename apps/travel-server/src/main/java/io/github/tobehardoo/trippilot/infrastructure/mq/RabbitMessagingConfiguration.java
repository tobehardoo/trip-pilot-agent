...
public class RabbitMessagingConfiguration {
    ...
    static final String AGENT_DIALOG_EVENT_QUEUE = "agent.dialog.event.queue";
    ...
    @Bean
    Queue agentDialogEventQueue() {
        return durableQueue(
                AGENT_DIALOG_EVENT_QUEUE,
                "agent.dialog.event.dead"
        );
    }
    ...
    @Bean
    Binding agentDialogAskUserBinding(
            Queue agentDialogEventQueue,
            DirectExchange planningEventExchange
    ) {
        return BindingBuilder.bind(agentDialogEventQueue)
                .to(planningEventExchange)
                .with("agent.ask-user");
    }

    @Bean
    Binding agentDialogStepBinding(
            Queue agentDialogEventQueue,
            DirectExchange planningEventExchange
    ) {
        return BindingBuilder.bind(agentDialogEventQueue)
    ...
    @Bean
    Binding agentDialogCompletedBinding(
            Queue agentDialogEventQueue,
            DirectExchange planningEventExchange
    ) {
        return BindingBuilder.bind(agentDialogEventQueue)
                .to(planningEventExchange)
                .with("agent.completed");
    }

    @Bean
    Binding agentDialogRunFinishedBinding(
            Queue agentDialogEventQueue,
            DirectExchange planningEventExchange
    ) {
        return BindingBuilder.bind(agentDialogEventQueue)
                .to(planningEventExchange)
                .with("agent.run-finished");
    }
    ...
    @Bean
    Binding agentDialogDeadLetterBinding(
            Queue planningDeadLetterQueue,
            TopicExchange planningDeadLetterExchange
    ) {
        return BindingBuilder.bind(planningDeadLetterQueue)
